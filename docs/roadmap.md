...
| **M2** | Transfers — up and download including folders, multipart, a queue with progress, cancel and retry | **in progress** — download now goes all the way: one object (`XONHO-0007`), a queue to carry many (`XONHO-0028`), and a whole folder that arrives as a folder (`XONHO-0034`). Upload is still one file at a time, and multipart — with it pause and resume — is the milestone's remaining weight |
...
| `XONHO-0034` | A folder that arrives as a folder: a folder row or any set of ticked rows fetched in one gesture, its prefix structure rebuilt as real directories (ADR-0005 extends the key→filename scheme to key→path), and one answer able to settle every name already taken at the destination |built; the subtree proven path for path and byte for byte against a real service, and one answer proven to settle a second whole fetch; **download only** — folder *upload* is not in it |
