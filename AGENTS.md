...
1. **Crate boundaries.** `caithung-core` owns all product logic and never
   depends on UI. `caithung-gui` never depends on `aws-sdk-s3` directly — if it
...
2. **Nothing that touches the network runs on the render thread.** AWS calls
   ...
   demonstrated in `caithung-gui/src/main.rs` (M0 spike).
...
7. **TDD for `caithung-core`.** The GUI may be exploratory; the core may not.

## Conventions

- Everything in the repo is **English**: code, comments, docs, commits, issues.
- Conventional Commits with a task ID in the scope: `feat(THUNG-0001): ...`.
  Small housekeeping may use the owner's global `OPS-NNNN` sequence.
...
- **Milestone: M0** — UI-stack spike. `caithung-gui` is the spike binary:
...
- CI builds Windows + macOS on every push/PR and uploads the spike binaries as
  artifacts (`caithung-spike-*`) for on-device M0b measurement.
